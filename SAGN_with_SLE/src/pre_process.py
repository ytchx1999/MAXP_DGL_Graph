...
def prepare_data(device, args, probs_path, stage=0, load_embs=False, load_label_emb=False, subset_list=None):
    ...
    if args.inductive:
        ...
        if load_embs and os.path.exists(emb_path):
            ...
            feats_train = neighbor_average_features_by_chunks(g_train, feat[g_train.ndata[dgl.NID]], args, style=feat_averaging_style,
                                                              stats=args.dataset not in ["ogbn-mag", "ogbn-papers100M"], memory_efficient=args.memory_efficient)
        ...
        if (stage == 0) and load_label_emb and os.path.exists(label_emb_path):
            ...
            if label_emb is not None:
                ...
                label_emb_train = neighbor_average_features_by_chunks(g_train, label_emb_train, args, style=label_averaging_style, stats=args.dataset not in [
                                                                      "ogbn-mag", "ogbn-papers100M"], memory_efficient=args.memory_efficient)
                label_emb = neighbor_average_features_by_chunks(g, label_emb, args, style=label_averaging_style, stats=args.dataset not in [
                                                                "ogbn-mag", "ogbn-papers100M"], memory_efficient=args.memory_efficient)
                if args.dataset in ["ppi", "yelp"]:
