...
def time_randomwalk(graph, args):
    ...
    print("Run dataset {} {} trials, mean run time: {:.3f}s".format(args.dataset, args.runs, cost_time_avg), flush=True)
...
if __name__ == '__main__':
    ...
    if args.task == 'train':
        print("Perform training node2vec model", flush=True)
        train_node2vec(graph, eval_set, args)
    elif args.task == 'time':
        print("Timing random walks", flush=True)
